...
def bet_contest_list(request):
    contests = Contest.objects.filter(
        is_active=True,
        status__in=["upcoming", "active", "final"],
    ).order_by("-date")
...
def place_bet_view(request, option_id):
    """
    El lock atomico esta en svc_place_bet (select_for_update).
    No duplicar @transaction.atomic aqui.
    """
    ...
    if error:
        ...
        messages.success(
            ...
            f"Ganancia potencial S/{bet.potential_win}. Saldo: S/{wallet.balance:.2f}.",
        )
    return redirect("bets:lobby", slug=opt.contest.slug)
...
def my_bets(request):
    ...
    return render(request, "bets/my_bets.html", {
        ...
        "resolved": base_qs.exclude(status="pending"),
    })